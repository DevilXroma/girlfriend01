...
function buildSection(sectionId) {
  ...
  const center = {
    ...
    color: section.color,
    icon: section.icon,
    x: 0, y: centerY, homeX: 0, homeY: centerY,
    vx: 0, vy: 0,
    r: mobile ? 31 : 42,
    sectionId: section.id,
    tags: []
  ...
  const rx = Math.min(width * (mobile ? 0.30 : 0.42), mobile ? 140 : 560);
  const ry = Math.min(height * (mobile ? 0.23 : 0.36), mobile ? 165 : 350);

  list.forEach((item, index) => {
    const angle = -Math.PI / 2 + (index / count) * Math.PI * 2;
    const lane = mobile ? (0.78 + (index % 3) * 0.09) : (0.78 + (index % 4) * 0.11);
    const x = Math.cos(angle) * rx * lane;
    const y = centerY + Math.sin(angle) * ry * lane;
    nodes.push({
      id: item.id,
      type: 'item',
      title: item.title,
      ...
      x, y, homeX: x, homeY: y,
      vx: 0, vy: 0,
      r: mobile ? 11 + Math.min((item.tags || []).length, 3) : 18 + Math.min((item.tags || []).length, 4),
      sectionId: section.id,
      tags: item.tags || []
    });
  });
...
function physics() {
  ...
  nodes.forEach((node) => {
    ...
    node.vx += (node.homeX - node.x) * (mobile ? 0.012 : 0.006);
    node.vy += (node.homeY - node.y) * (mobile ? 0.012 : 0.006);
    if (focus && !related.has(node.id) && !mobile) {
  ...
  links.forEach((link) => {
    ...
    const target = link.type === 'parent' ? (mobile ? 95 : 190) : link.type === 'tag' ? (mobile ? 130 : 250) : (mobile ? 155 : 330);
    const force = (dist - target) * (mobile ? 0.0014 : 0.00075) * (link.weight || 1);
  ...
  for (let i = 0; i < nodes.length; i += 1) {
    for (let j = i + 1; j < nodes.length; j += 1) {
      ...
      const dy = b.y - a.y;
      const dist = Math.hypot(dx, dy) || 1;
      const min = a.r + b.r + (mobile ? 18 : (a.type.includes('section') || b.type.includes('section') ? 74 : 42));
      if (dist < min) {
        const push = (min - dist) * (mobile ? 0.018 : 0.011);
  ...
  nodes.forEach((node) => {
    ...
    node.vy *= mobile ? 0.78 : 0.86;
    node.x += node.vx;
    node.y += node.vy;
    const pad = mobile ? 28 : 64;
    node.x = Math.max(-width / 2 + pad, Math.min(width / 2 - pad, node.x));
    node.y = Math.max(-height / 2 + pad, Math.min(height / 2 - pad, node.y));
  });
}
...
function drawLabel(node, radius, active, selected) {
  const mobile = isMobile();
  const isItem = node.type === 'item';
  const maxChars = mobile ? (isItem ? 11 : 15) : 26;
  const size = mobile ? (isItem ? 9 : 12) : (isItem ? 12 : 16);
  if (mobile && isItem && !active) return;
  ctx.fillStyle = '#fff';
  ctx.font = `${isItem ? 800 : 900} ${selected ? size + 1 : size}px Inter, sans-serif`;
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.fillText(shortLabel(node.title, maxChars), node.x, node.y - radius - (mobile ? 10 : 15));
}
...
function draw() {
  ...
  links.forEach((link) => {
    ...
    ctx.save();
    ctx.globalAlpha = active ? (link.type === 'tag' ? (mobile ? 0.14 : 0.25) : (mobile ? 0.42 : 0.55)) : 0.03;
    ctx.strokeStyle = link.type === 'tag' ? 'rgba(255,255,255,.52)' : link.a.color;
    ctx.lineWidth = mobile ? (link.type === 'tag' ? 0.7 : 1.3) : (link.type === 'tag' ? 1 : 2.2);
    ctx.shadowColor = link.a.color;
    ctx.shadowBlur = active ? (mobile ? 5 : 12) : 0;
    ctx.beginPath();
  ...
  nodes.forEach((node) => {
    ...
    const radius = node.r + (selected ? (mobile ? 3 : 7) : 0);
    ctx.save();
    ctx.globalAlpha = active ? 1 : (mobile ? 0.11 : 0.17);
    ctx.shadowColor = node.color;
    ctx.shadowBlur = node.type === 'item' ? (mobile ? 7 : 16) : (mobile ? 16 : 32);
    ctx.fillStyle = node.color;
    ctx.beginPath();
    ctx.arc(node.x, node.y, radius, 0, Math.PI * 2);
    ctx.fill();
    ctx.shadowBlur = 0;
    drawLabel(node, radius, active, selected);
    if (node.icon) {
      ctx.fillStyle = 'rgba(0,0,0,.45)';
      ctx.font = `900 ${mobile ? 14 : 20}px Inter, sans-serif`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(node.icon, node.x, node.y + 1);
    }
    if (node.type !== 'item' && !mobile) {
    ...
    if (selected) {
      ...
      ctx.lineWidth = mobile ? 1 : 1.6;
      ctx.beginPath();
      ctx.arc(node.x, node.y, radius + (mobile ? 5 : 10), 0, Math.PI * 2);
      ctx.stroke();
    }
    ctx.restore();
  });
...
function currentTags() { return tagsInput.value.split(',').map((tag) => tag.trim()).filter(Boolean); }
...
editForm.addEventListener('submit', (event) => {
  ...
  if (mode === 'edit-section') {
    ...
    if (section) { section.title = title; section.description = comment; }
  }
  if (mode === 'add-item') {
    const sectionId = sectionSelect.value || state.activeSectionId || sections[0]?.id;
    const item = { id: `item-${makeId(title)}-${Date.now().toString(36)}`, sectionId, title, comment, tags: tagsInput.value.split(',').map((tag) => tag.trim()).filter(Boolean) };
    items.push(item);
    state.view = 'section';
